...
import FinderPostTableRow from './FinderPostTableRow';
import Form from "react-bootstrap/Form";
import FormControl from "react-bootstrap/FormControl";
import Button from "react-bootstrap/Button";

class FinderPostList extends Component {
    constructor(props) {
        super(props)
    ...
    searchByName() {
      const searchedEmployees = (this.state.allFinderPosts.filter(i => (
      ...
      if (searchedEmployees.length === 0) {
        this.setState({finderPosts: this.state.allFinderPosts,
           searchbyname: '', searchresultmsg: 'Invalid finder post name or description'});
      } else {
        this.setState({finderPosts: searchedEmployees});
      }
    }
    componentDidMount() {
      axios.get('http://localhost:4000/finders/get-finderPost')
        .then(res => {
          this.setState({
            finderPosts: res.data
          });
          // console.log('e',this.state.employees);
          this.setState({allFinderPosts: this.state.finderPosts});
        })
        .catch((error) => {
          console.log(error);
        })
      ...
      DataTable() {
        return this.state.finderPosts.map((res, i) => {
          return <FinderPostTableRow obj={res} key={i} />;
        });
      }
    render() {
        return (
