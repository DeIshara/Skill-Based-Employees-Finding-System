...
class CategoryList extends Component {
    ...
    searchByName() {
      const searchedEmployees = (this.state.allCategories.filter(i => (
        (i.categoryname.toLowerCase().includes(this.state.searchbyname.toLowerCase())) || 
        (i.categorydescription.toLowerCase().includes(this.state.searchbyname.toLowerCase()))
      )));
      if (searchedEmployees.length === 0) {
        this.setState({categories: this.state.allCategories,
           searchbyname: '', searchresultmsg: 'Invalid category name',});
      } else {
        this.setState({categories: searchedEmployees});
      }
    }
      ...
      componentDidMount() {
        axios.get('http://localhost:4000/categories/get-categories')
          .then(res => {
            ...
            this.setState({allCategories: this.state.categories});
          })
          .catch((error) => {
            console.log(error);
          })
