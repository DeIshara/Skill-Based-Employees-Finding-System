...
import EmployeePostTableRow from './EmployeePostTableRow';
import Form from "react-bootstrap/Form";
import FormControl from "react-bootstrap/FormControl";
import Button from "react-bootstrap/Button";

class EmployeePostList extends Component {
    constructor(props) {
        super(props)
    ...
    searchByName() {
      const searchedEmployees = (this.state.allEmployeePost.filter(i => (
      ...
      if (searchedEmployees.length === 0) {
        this.setState({employeePost: this.state.allEmployeePost,
           searchbyname: '', searchresultmsg: 'Invalid employee post name or description',});
      } else {
        this.setState({employeePost: searchedEmployees});
      }
    }
    componentDidMount() {
      axios.get('http://localhost:4000/employees/get-employeePost')
        .then(res => {
          this.setState({
            employeePost: res.data
          });
          // console.log('e',this.state.employees);
          this.setState({allEmployeePost: this.state.employeePost});
        })
        .catch((error) => {
          console.log(error);
        })
      ...
      DataTable() {
        return this.state.employeePost.map((res, i) => {
          return <EmployeePostTableRow obj={res} key={i} />;
        });
      }
    render() {
        return (
